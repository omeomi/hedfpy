...
class EyeSignalOperator(Operator):
	...
	def __init__(self, input_object, **kwargs):
		...
		if hasattr(self, 'eyelink_blink_data'):
			...
			self.blink_starts_EL = np.array(self.eyelink_blink_data['start_timestamp'])[(self.blink_dur_EL<4000) & (self.eyelink_blink_data['start_timestamp']>=self.timepoints[0])] - self.timepoints[0]
			self.blink_ends_EL = np.array(self.eyelink_blink_data['end_timestamp'])[(self.blink_dur_EL<4000) & (self.eyelink_blink_data['start_timestamp']>=self.timepoints[0])] - self.timepoints[0]
